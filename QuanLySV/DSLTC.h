...
public:
	...
	string checkTrungDS_LTC(LopTC* ltc, int viTri);

	void huyLopTCTheoKhoaVaHK_LTC(string khoa, int HK);

	int checkLopTheoKhoaVaHK_LTC(string khoa, int HK);
	...
	void xuatDSTheoTrang_LTC(treeMH DSMH, Subject arrMH[], int soLuongMH, int& viTriChon, int tongSoDong, thaoTac& hDLTC, DSSV SV);
	...
	void dangKyTheoLTC(Subject arrMH[], int soLuongMH, string maSV, string khoa, int HK, thaoTac& hDLTC);

	void inputDkLTC(Subject arrMH[], int soLuongMH, string str1, string str2, string str3, DSSV SV, thaoTac& hDLTC);

	void huyLTC(string str1, string str2, thaoTac& hDLTC);

	void xuatDSDK(DSSV SV, thaoTac& hDLTC, Subject arrMH[], int soLuongMH);
	...
	void xuatDiemLTC(Subject arrMH[], int soLuongMH, DSSV SV, int tongSoDong, thaoTac& hDLTC);

	void xuatDiem1Trang1SV(LopTC* loptc[], Subject arrMH[], int soLuongMon, int batDau, int ketThuc, Table Bang, string maSV);

	void xuatDiemTheoTrang1SV(LopTC* loptc[], Subject arrMH[], int soLuongMH, int tongSoDong, thaoTac& hDLTC, string masv, string hoTen);

	void xuatDiem1SV(Subject arrMH[], int soLuongMH, DSSV SV, thaoTac& hDMH);
	...
	void menuLTC(treeMH DSMH, DSSV SV);

	void menuDiem(treeMH DSMH, DSSV SV);
};
...
void LopTC::setNienKhoa(string khoa) {
	this->nienKhoa = khoa;
}
...
string DSLTC::checkTrungDS_LTC(LopTC* ltc, int viTri) {
	if (this->isNull_LTC()) return "-1";
...
int DSLTC::checkLopTheoKhoaVaHK_LTC(string khoa, int HK) {
	for (int i = 0; i < n; i++) {
...
void DSLTC::huyLopTCTheoKhoaVaHK_LTC(string khoa, int HK) {
	for (int i = 0; i < n; i++) {
...
void DSLTC::loadDataDS_LTC() {
	...
	if (fileIn.is_open()) {
		...
		for (int i = 0; i < soLuong; i++) {
			...
			getline(fileIn, tempStr, ','); lopTC[n]->setNienKhoa(tempStr);
...
void DSLTC::xuatDS1Trang_LTC(LopTC* loptc[], Subject arrMH[], int soLuongMH, int batDau, int ketThuc, Table Bang, int checkOut, string masv) {
	...
	for (int i = batDau; i < soDong; i++) {
		...
		outtextxy(x + textwidth(convertStringToChar(string("|"))), y,
			convertStringToChar(string((Bang.getCols(4)->getChieuRong() - textwidth(convertStringToChar(string("|")))) / textwidth(convertStringToChar(string(" "))), ' ')));
		// xuat KHOA moi
		outtextxy(x + (Bang.getCols(4)->getChieuRong() / 2 - textwidth(convertStringToChar(loptc[i]->getNienKhoa())) / 2), y, convertStringToChar(loptc[i]->getNienKhoa()));
		x += Bang.getCols(4)->getChieuRong();
...
void DSLTC::xuatDSTheoTrang_LTC(treeMH DSMH, Subject arrMH[], int soLuongMH, int& viTriChon, int tongSoDong, thaoTac& hDLTC, DSSV SV) {
		...
		if (ismouseclick(WM_LBUTTONDOWN)) {
			...
			if (tableLeft <= x && x <= tableRightLopTC && tableTop + kcY <= y && y <= (ketThuc - (trangHienTai - 1) * maxDong) * kcY + tableTop + kcY) {
				...
				if (lopTC[indexXemDiem]->getSoSVDK() == 0) {
					...
					_menuxemDSSV();
					int xoa = 0;
					DSSV1LTC(convertIntToString(maLop), monHoc, convertIntToString(lopTC[index]->getSoSVDK()), lopTC[index]->getNienKhoa(),
			...
			if (tableLeft <= x && x <= tableRightLopTC && tableTop + kcY <= y && y <= (ketThuc - (trangHienTai - 1) * maxDong) * kcY + tableTop + kcY) {
				...
				else if (lopTC[viTriChon]->getTrangThai() == true) {
					MessageBox(
						NULL,
						(LPCWSTR)convertCharArrayToLPCWSTR("LOP DA BI HUY. KHONG THE SUA !!!"),
						(LPCWSTR)convertCharArrayToLPCWSTR("THONG BAO"),
						MB_ICONERROR | MB_OK | MB_DEFAULT_DESKTOP_ONLY
					);
					hDLTC = XUAT;
				...
				inputNhapLTC(convertIntToString(lopTC[viTriChon]->getMaLopTC()), lopTC[viTriChon]->getMaMH(), lopTC[viTriChon]->getNienKhoa(),
...
void DSLTC::inputNhapLTC(string str1, string str2, string str3, string str4,
	...
	input[2] = new Input(str3, 2, "Nien Khoa", 9, 0, 600, 1100, 340, 380, graynhat, 0, 3);
			...
			if (ismouseclick(WM_LBUTTONDOWN)) {
				...
				if (425 <= x && x <= 520 && 695 <= y && y <= 734) {
					...
					if (input[0]->checkRong() == true && input[1]->checkRong() == true && input[2]->checkRong() == true && input[3]->checkRong() == true && input[4]->checkRong() == true &&
						...
						lop->setNienKhoa(input[2]->getContentStr());
						...
						string nien_khoa = checkTrungDS_LTC(lop, -1);
						if (DSMH.checkTrung(lop->getMaMH()) == false) {
							...
							string str2 = nien_khoa; // convertIntToString(nien_khoa);
							str1 = str1 + str2;
							MessageBox(
								NULL,
		...
		while (1) {
			...
			if (ismouseclick(WM_LBUTTONDOWN)) {
				...
				if (1057 <= x && x <= 1137 && 695 <= y && y <= 734) {
					...
					lop->setSVMin(atoi(input[5]->getContent()));
					lop->setSVMax(atoi(input[6]->getContent()));
					string nien_khoa = checkTrungDS_LTC(lop, viTriChon);
					if (nien_khoa != "-1") {

						string str1 = "LOP TIN CHI DA CO TAI NIEN KHOA ";
						string str2 = nien_khoa;
						str1 = str1 + str2;
						MessageBox(
							NULL,
							(LPCWSTR)convertCharArrayToLPCWSTR(str1.c_str()),
...
void DSLTC::dangKyTheoLTC(Subject arrMH[], int soLuongMH, string _maSV, string khoa, int HK, thaoTac& hDDK) {
	int checkOut = 1;
	int tongSoDong = 0;
...
void DSLTC::inputDkLTC(Subject arrMH[], int soLuongMH, string str1, string str2, string str3, DSSV SV, thaoTac& hDDK) {
	Input* input[3];
	input[0] = new Input(str1, 2, "Ma SV", 15, 0, 570, 1100, 260, 310, graynhat, 0, 3);
	input[1] = new Input(str2, 2, "Nien Khoa", 10, 0, 570, 1100, 360, 410, graynhat, 0, 3);
			...
			if (417 <= x && x <= 512 && 709 <= y && y <= 748) {
				...
				if (input[0]->checkRong() == true && input[1]->checkRong() == true && input[2]->checkRong() == true) {
					...
					if (tmp == NULL) {
						...
						cout << "siuuuuu";
						cleardevice();
						dangKyTheoLTC(arrMH, soLuongMH, ma_sv, nien_Khoa, hoc_Ky, hDDK);
						//writeDataDS_DK();
						//writeDataDS_LTC();
						break;
					}
...
void DSLTC::huyLTC(string str1, string str2, thaoTac& hDLTC) {
	Input* input[2];
	input[0] = new Input(str1, 2, "Nien khoa", 50, 0, 570, 1100, 300, 350, graynhat, 0, 4);
...
void DSLTC::xuatDSDK(DSSV SV, thaoTac& hDLTC, Subject arrMH[], int soLuongMH) {
		...
		if (ismouseclick(WM_LBUTTONDOWN)) {
			...
			if (700 <= x && x <= 1100 && 300 <= y && y <= 350) {
				...
				if (tim_LTC(maLop) < 0) {
					...
					tieuDeDSSV();
					_menuxemDSSV();
					DSSV1LTC(convertIntToString(maLop), monHoc, convertIntToString(lopTC[viTriLTC]->getSoSVDK()), lopTC[viTriLTC]->getNienKhoa(),
...
void DSLTC::xuatDiemTheoTrang1LTC(LopTC* loptc, DSSV SV, int tongSoDong, thaoTac& hDLTC) {
	...
	while (ok) {
		...
		if (ismouseclick(WM_LBUTTONDOWN)) {
			...
			else if (tableLeft <= x && x <= 1380 && tableTop + kcY <= y && y <= (ketThuc - (trangHienTai - 1) * maxDong) * kcY + tableTop + kcY) {
				viTriChon = (y - tableTop - kcY) / kcY + (trangHienTai - 1) * maxDong;
...
void DSLTC::xuatDiemLTC(Subject arrMH[], int soLuongMH, DSSV SV, int tongSoDong, thaoTac& hDLTC) {
...
void DSLTC::xuatDiemTheoTrang1SV(LopTC* loptc[], Subject arrMH[], int soLuongMH, int tongSoDong, thaoTac& hDLTC, string maSV, string hoTen) {
...
void DSLTC::xuatDiem1SV(Subject arrMH[], int soLuongMH, DSSV SV, thaoTac& hDLTC) {
			...
			if (500 <= x && x <= 900 && 200 <= y && y <= 225) {
				...
				if (Sv == nullptr) {
					...
					if (soLuongLop > 0) {
						...
						strho = strho + strten;
						cleardevice();
						xuatDiemTheoTrang1SV(loptc, arrMH, soLuongMH, soLuongLop, hDLTC, maSV, strho);
						if (hDLTC == BACK) break;
					}
					else {
						MessageBox(
...
void DSLTC::menuLTC(treeMH DSMH, DSSV sv) {
	...
	while (ok) {
		...
		case XUAT: {
			...
			_menuLTC();
			soLuong = n;
			xuatDSTheoTrang_LTC(DSMH, arrMH, soLuongMH, viTriChon, soLuong, hDLTC, sv);
		...
		case DANGKY: {
			...
			inputDkLTC(arrMH, soLuongMH, "", "", "", sv, hDLTC);
			hDLTC = XUAT;
			break;
		}
		case HUY: {
		...
		case XUATDK: {
			...
			xuatDSDK(sv, hDLTC, arrMH, soLuongMH);
			cleardevice();
			_menuLTC();
			soLuong = n;
			xuatDSTheoTrang_LTC(DSMH, arrMH, soLuongMH, viTriChon, soLuong, hDLTC, sv);
...
void DSLTC::menuDiem(treeMH DSMH, DSSV SV) {
	...
	while (ok) {
		...
		case XUAT: {
			...
			_menuDiem();
			soLuong = n;
			xuatDiemLTC(arrMH, soLuongMH, SV, soLuong, hDLTC);
			break;
		}
		case DIEM1SV: {
			//cleardevice();
			...
			menuInputDiem1SV();
			xuatDiem1SV(arrMH, soLuongMH, SV, hDLTC);
			cleardevice();
			hDLTC = XUAT;
			break;
		}
